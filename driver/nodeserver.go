...
func (sym *Driver) NodeStageVolume(ctx context.Context, req *csi.NodeStageVolumeRequest) (*csi.NodeStageVolumeResponse, error) {
	if req.VolumeId == "" {
	...
	deviceName, ok := req.GetPublishContext()[sym.config.PublishInfoDeviceNameParam]
	if !ok {
		return nil, status.Error(codes.InvalidArgument, "Could not find the device by name")
	}
	...
	source := filepath.Join("/dev", deviceName)
	...
	if !mounted {
		if err := sym.mounter.Mount(filepath.Join("/dev", deviceName), target, fsType, options...); err != nil {
			return nil, status.Error(codes.Internal, err.Error())
		}
	} else {
...
func (sym *Driver) NodeUnstageVolume(ctx context.Context, req *csi.NodeUnstageVolumeRequest) (*csi.NodeUnstageVolumeResponse, error) {
	if req.VolumeId == "" {
...
func (sym *Driver) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	if req.VolumeId == "" {
	...
	case *csi.VolumeCapability_Block:
		err = sym.nodePublishVolumeForBlock(req, devicePath, options, log)
...
func (sym *Driver) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	if req.VolumeId == "" {
...
func (sym *Driver) NodeGetCapabilities(ctx context.Context, req *csi.NodeGetCapabilitiesRequest) (*csi.NodeGetCapabilitiesResponse, error) {
...
func (sym *Driver) NodeGetInfo(ctx context.Context, req *csi.NodeGetInfoRequest) (*csi.NodeGetInfoResponse, error) {
...
// the given volume.
func (sym *Driver) NodeGetVolumeStats(ctx context.Context, req *csi.NodeGetVolumeStatsRequest) (*csi.NodeGetVolumeStatsResponse, error) {
	if req.VolumeId == "" {
...
func (sym *Driver) NodeExpandVolume(ctx context.Context, req *csi.NodeExpandVolumeRequest) (*csi.NodeExpandVolumeResponse, error) {
...
func (sym *Driver) nodePublishVolumeForFileSystem(req *csi.NodePublishVolumeRequest, mountOptions []string, log *logrus.Entry) error {
...
func (sym *Driver) nodePublishVolumeForBlock(req *csi.NodePublishVolumeRequest, devicePath string, mountOptions []string, log *logrus.Entry) error {
	target := req.TargetPath
	...
	if !mounted {
		log.Info("mounting the volume")
		if err := sym.mounter.Mount(devicePath, target, "", mountOptions...); err != nil {
