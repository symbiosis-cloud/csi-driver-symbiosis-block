...
func (sym *symbiosisOp) checkPVCapacityValid(ctx context.Context, volID string) (bool, error) {
	volumePath := sym.getVolumePath(volID)
	_, fscapacity, _, _, _, _, err := fs.FsInfo(volumePath)
	if err != nil {
		return false, fmt.Errorf("failed to get capacity info: %+v", err)
	}

	volume, _, err := sym.client.GetVolumeByID(ctx, volID)
	if err != nil {
		return false, err
	}
	volumeCapacity := volume.StorageGiB
...
func (sym *symbiosisOp) checkPVUsage(volID string) (bool, error) {
	volumePath := sym.getVolumePath(volID)
...
func (sym *symbiosisOp) doHealthCheckInControllerSide(ctx context.Context, volID string) (bool, string) {
	volumePath := sym.getVolumePath(volID)
	...
	capValid, err := sym.checkPVCapacityValid(ctx, volID)
	if err != nil {
		return false, err.Error()
	}
	...
	available, err := sym.checkPVUsage(volID)
	if err != nil {
		return false, err.Error()
	}
...
func (sym *symbiosisOp) doHealthCheckInNodeSide(volID string) (bool, string) {
	volumePath := sym.getVolumePath(volID)
