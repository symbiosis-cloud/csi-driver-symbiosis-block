...
func (sym *symbiosisOp) NodeStageVolume(ctx context.Context, req *csi.NodeStageVolumeRequest) (*csi.NodeStageVolumeResponse, error) {
	if req.VolumeId == "" {
	...
	log := sym.log.WithFields(logrus.Fields{
	...
	volumeName := ""
	if volName, ok := req.GetPublishContext()[sym.config.PublishInfoVolumeNameParam]; !ok {
	...
	if noFormat {
		...
		formatted, err := sym.mounter.IsFormatted(source)
		if err != nil {
			return nil, err
		}
		...
		if !formatted {
			...
			if err := sym.mounter.Format(source, fsType); err != nil {
	...
	mounted, err := sym.mounter.IsMounted(target)
	if err != nil {
		return nil, err
	}

	if !mounted {
		if err := sym.mounter.Mount(source, target, fsType, options...); err != nil {
			return nil, status.Error(codes.Internal, err.Error())
		}
	} else {
...
func (sym *symbiosisOp) NodeUnstageVolume(ctx context.Context, req *csi.NodeUnstageVolumeRequest) (*csi.NodeUnstageVolumeResponse, error) {
	if req.VolumeId == "" {
	...
	mounted, err := sym.mounter.IsMounted(req.StagingTargetPath)
	if err != nil {
		return nil, err
	}
	...
	if mounted {
		...
		err := sym.mounter.Unmount(req.StagingTargetPath)
		if err != nil {
			return nil, err
		}
	} else {
...
func (sym *symbiosisOp) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	if req.VolumeId == "" {
	...
	case *csi.VolumeCapability_Block:
		err = sym.nodePublishVolumeForBlock(req, options, log)
	case *csi.VolumeCapability_Mount:
		err = sym.nodePublishVolumeForFileSystem(req, options, log)
	default:
...
func (sym *symbiosisOp) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	if req.VolumeId == "" {
	...
	err := sym.mounter.Unmount(req.TargetPath)
	if err != nil {
		return nil, err
	}
...
func (sym *symbiosisOp) NodeGetCapabilities(ctx context.Context, req *csi.NodeGetCapabilitiesRequest) (*csi.NodeGetCapabilitiesResponse, error) {
	...
	sym.log.WithFields(logrus.Fields{
...
func (sym *symbiosisOp) NodeGetInfo(ctx context.Context, req *csi.NodeGetInfoRequest) (*csi.NodeGetInfoResponse, error) {
	sym.log.WithField("method", "node_get_info").Info("node get info called")
	return &csi.NodeGetInfoResponse{
		NodeId:            sym.config.NodeID,
		MaxVolumesPerNode: maxVolumesPerNode,

		// make sure that the driver works on this particular region only
		AccessibleTopology: &csi.Topology{
			Segments: map[string]string{
				"region": sym.config.Region,
			},
		},
	}, nil
}
...
// the given volume.
func (sym *symbiosisOp) NodeGetVolumeStats(ctx context.Context, req *csi.NodeGetVolumeStatsRequest) (*csi.NodeGetVolumeStatsResponse, error) {
	if req.VolumeId == "" {
	...
	mounted, err := sym.mounter.IsMounted(volumePath)
	if err != nil {
...
func (sym *symbiosisOp) NodeExpandVolume(ctx context.Context, req *csi.NodeExpandVolumeRequest) (*csi.NodeExpandVolumeResponse, error) {
	...
	mounter := mount.New("")
	devicePath, err := sym.mounter.GetDeviceName(mounter, volumePath)
	if err != nil {
...
func (sym *symbiosisOp) nodePublishVolumeForFileSystem(req *csi.NodePublishVolumeRequest, mountOptions []string, log *logrus.Entry) error {
	...
	if !mounted {
		...
		if err := sym.mounter.Mount(source, target, fsType, mountOptions...); err != nil {
...
func (sym *symbiosisOp) nodePublishVolumeForBlock(req *csi.NodePublishVolumeRequest, mountOptions []string, log *logrus.Entry) error {
	volumeName, ok := req.GetPublishContext()[sym.config.PublishInfoVolumeNameParam]
	if !ok {
		return status.Error(codes.InvalidArgument, fmt.Sprintf("Could not find the volume name from the publish context %q", sym.config.PublishInfoVolumeNameParam))
	}
	...
	if !mounted {
		log.Info("mounting the volume")
		if err := sym.mounter.Mount(source, target, "", mountOptions...); err != nil {
